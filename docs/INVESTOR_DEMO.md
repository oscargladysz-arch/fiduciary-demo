...
Investment Selection Records, one per plan and product, and one attachment
with the verbatim rule text, named by its content hash).
...
- **Investment Selection Record** (Packet view): one record per plan and
  product, generated by the build, file names carry the plan, page one is
  the decision summary and the signature block, the verbatim rule text is
  the attachment beside it. Print is scoped to pinned exhibits.
...
- The verbatim rule text was fetched by `python src/fetch_authority.py` on
  ...
  record quotes the rule paragraphs in its regulatory basis and cites the
  full text in Attachment A by its content hash. The demo script opens the
  panel once, at 0:40.
