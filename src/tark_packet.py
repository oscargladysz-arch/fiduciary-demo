...
from tark_display import RUBRIC_LABEL, display_copy, display_path_free, facts_by_cell, typed_headline
...
def build_packet(key: str, plan_key: str, out_dir: Path | None = None) -> Path:
    ...
    head.add_run(f"Record as of {record_as_of()}. DRAFT, generated from the record. "
                 "The decision memo for the same plan and product is a separate document.")

    # ---- one-page summary
    doc.add_heading("Summary", level=1)
    if m:
        doc.add_paragraph(f"Structural liquidity verdict: {m['verdict'].upper()} (dealing terms on record, plan-independent). "
                          f"Scenario verdict under this plan (ILLUSTRATIVE, default turnover assumptions): "
                          f"{(m.get('scenario_verdict') or 'not computable').upper()}.")
    if sel is None:
        doc.add_paragraph("Benchmark: no selection record.")
    ...
    doc.add_paragraph("Flags raised by the record (each restates a recorded value or verdict, with its source):")
    ...
    if sel is None:
        doc.add_paragraph("No selection record for this product.")
    else:
        sk = sel["slot_k"]
    ...
    doc.add_heading("Exhibit C. Fees and terms on record", level=1)
    doc.add_paragraph("Each line is the fact read from the cell, or the cell's status word when none is "
                      "on record. Full text and provenance are in the record.")
    t = doc.add_table(rows=1, cols=3)
    t.style = "Table Grid"
    h = t.rows[0].cells
    h[0].text, h[1].text, h[2].text = "Cell", "Headline", "Status"
    ...
    if cpath and cpath.exists():
        ...
        doc.add_paragraph(f"Cohort {co['label']} (n={co['n']}). {display_copy(fdoc.get('membership_rationale', ''))}")
        v29 = p["cells"]["2.9"].get("value")
        if v29:
            doc.add_paragraph(display_path_free(v29))
        for cv in co.get("caveats", []):
            doc.add_paragraph(display_copy(cv), style="List Bullet")
    else:
        doc.add_paragraph("No cohort record for this product.")
    ...
    doc.add_paragraph(f"Regulatory basis: {RULE_CITATION}, paragraphs {RULE['paragraphs']}, cited not paraphrased. "
                      f"Every figure above is a cited cell, a figure read from one, or a computation of the "
